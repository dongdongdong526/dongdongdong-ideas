...
out = Path(r"C:\Users\User\Desktop\清北目标与赚钱思考.docx")
...
def make_p(kind, text):
    ppr = f"<w:pPr>{style_map[kind]}</w:pPr>" if kind in style_map else ""
    runs = []
    for i, line in enumerate(str(text).split("\n")):
        if i:
            runs.append("<w:r><w:br/></w:r>")
        space = ' xml:space="preserve"' if kind == "code" or line.startswith(" ") or line.endswith(" ") else ""
        runs.append(f"<w:r><w:t{space}>{escape(line)}</w:t></w:r>")
    return f"<w:p>{ppr}{''.join(runs)}</w:p>"

body = "".join(make_p(k, t) for k, t in paragraphs)
